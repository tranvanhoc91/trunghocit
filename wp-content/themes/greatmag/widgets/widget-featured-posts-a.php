...
class Athemes_Featured_Posts_Type_A extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => __( '7 featured posts displayed in a grid', 'greatmag' ),
		...
		<div class="sticky-posts-blocks sticky-posts isotope-gallery">
			...
			<?php while ( $r->have_posts() ) : $r->the_post(); ?>
				<?php if( $counter != 2 ) {
					...
					$col = 'col-md-6';
					$title_size = 'big';					
					$meta = '<h5 class="post-meta"><a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '" class="author">' . esc_html( get_the_author() ) . '</a>  -  <a href="' . get_the_permalink() . '" class="date">' . get_the_date() . '</a></h5>';
	...
	?>
        ...
        <select data-placeholder="<?php echo __('Select five posts to display in this widget', 'greatmag'); ?>" multiple="multiple" name="<?php echo $this->get_field_name('posts_dropdown'); ?>" id="<?php echo $this->get_field_id('posts_dropdown'); ?>" class="widefat chosen-dropdown-5">
		<?php
		global $post;
